...
const GenesAndMutationFrequencyAnalysisTool: React.FC = () => {
  ...
  const cohortId = useCoreSelector((state: any) =>
  ...
  const handleSurvivalPlotToggled = useDeepCompareCallback(
    (symbol: string, name: string, field: string) => {
      if (comparativeSurvival && comparativeSurvival?.symbol === symbol) {
        setComparativeSurvival(undefined as unknown as ComparativeSurvival);
      } else {
        setComparativeSurvival({
          symbol: symbol,
          name: name,
  ...
  const [tableXPosition, setTableXPosition] = useState<number>(
      ...
      >
        <div className="flex gap-4 m-4">
          ...
          <Tabs
            ...
            onChange={handleTabChanged as any}
            keepMounted={false}
          >
            <Tabs.List>
