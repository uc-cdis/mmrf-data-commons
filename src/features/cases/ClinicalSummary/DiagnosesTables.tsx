...
const DiagnosesTables = ({ data }: { data: Diagnoses }): JSX.Element => {
  const formatDataForDiagnoses = (data: Diagnoses) => {
    ...
    const tableData = {
      ...
      diagnosis_uuid,
      classification_of_tumor,
      age_at_diagnosis: ageDisplay(age_at_diagnosis as number),
      days_to_last_follow_up: ageDisplay(days_to_last_follow_up as number),
      days_to_last_known_disease_status: ageDisplay(
        days_to_last_known_disease_status as number,
      ),
      days_to_recurrence: ageDisplay(days_to_recurrence as number),
      last_known_disease_status,
      morphology,
      primary_diagnosis,
      prior_malignancy,
  ...
  const formatTreatmentData = (diagnoses: Diagnoses) => {
    const treatments: any = diagnoses.treatments;
    const treatmentTableData = treatments
      .slice()
      .sort(
        (a: any, b: any) =>
          a.days_to_treatment_start - b.days_to_treatment_start,
      )
      .map((diagnosis: any) => ({
        ...
        days_to_treatment_start: ageDisplay(
          diagnosis.days_to_treatment_start as number,
        ),
      }));
