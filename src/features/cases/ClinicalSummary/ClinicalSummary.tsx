...
export const ClinicalSummary = ({
  ...
  const formatDataForDemographics = () => {
    ...
    const demographicData: Record<string, any> = {
      ...
      gender,
      race,
      days_to_birth: ageDisplay(days_to_birth as number),
      days_to_death: ageDisplay(days_to_death as number),
      vital_status,
    };
  ...
  const CountComponent = ({ count }: { count: number }) => (
    <span
      className={`h-[11px] w-4 bg-accent-darker text-base-lightest text-xs
      font-medium px-1.5 py-0.5 ml-1 rounded-sm`}
    >
      {count}
    </span>
  );
    ...
    <div className="max-w-full">
      ...
      <Tabs
        ...
        classNames={{
          ...
          tab: `text-secondary-light font-bold font-heading text-sm px-4 py-1 mr-2
            data-active:bg-nci-cyan-lightest data-active:border-2 data-active:border-primary
            data-active:text-secondary-contrast-max`,
        }}
        styles={(theme) => ({
          tab: {
              ...
              dataInfo={
                follow_ups.length > 1
                  ? follow_ups
                      ...
                      .sort(
                        (a: any, b: any) =>
                          a.days_to_follow_up - b.days_to_follow_up,
                      )
                  : follow_ups
              }
